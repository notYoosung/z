...
minetest.register_craftitem(modname .. ":ctf_ranged_ammo", {
	description = "Ammo\nUsed to reload guns",
...
local function process_ray(ray, user, look_dir, def)
	local hitpoint = ray:hit_object_or_node({
		node = function(ndef)
			return ndef and ((ndef.walkable == true and ndef.pointable == true) or ndef.groups.liquid)
		end,
		object = function(obj)
			return (obj:is_player() and obj ~= user)-- or obj:get_luaentity()
		end
	})
...
function ctf_ranged.simple_register_gun(name, def)
	minetest.register_tool(rawf.also_register_loaded_tool(name, {
		...
		ammo = def.ammo or modname .. ":ctf_ranged_ammo",
		rounds = def.rounds,
...
function ctf_ranged.show_scope(name, item_name, fov_mult)
	...
	hud:add(player, modname .. ":ctf_ranged_scope", {
		hud_elem_type = "image",
...
function ctf_ranged.hide_scope(name)
	...
	hud:remove(name, modname .. ":ctf_ranged_scope")
	player:set_fov(0)
...
ctf_ranged.simple_register_gun(modname .. ":ctf_ranged_pistol", {
	type = "pistol",
...
ctf_ranged.simple_register_gun(modname .. ":ctf_ranged_rifle", {
	type = "rifle",
...
ctf_ranged.simple_register_gun(modname .. ":ctf_ranged_shotgun", {
	type = "shotgun",
...
ctf_ranged.simple_register_gun(modname .. ":ctf_ranged_smg", {
	type = "smg",
...
ctf_ranged.simple_register_gun(modname .. ":ctf_ranged_sniper", {
	type = "sniper",
...
ctf_ranged.simple_register_gun(modname .. ":ctf_ranged_sniper_magnum", {
	type = "sniper",
