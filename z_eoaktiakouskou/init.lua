...
local BOW_CHARGE_TIME_HALF = 20000 -- bow level 1
local BOW_CHARGE_TIME_FULL = 50000 -- bow level 2 (full charge)
...
function z_bows.shoot_arrow(arrow_item, pos, dir, yaw, shooter, power, damage, is_critical, bow_stack, collectable)
...
local function player_shoot_arrow(itemstack, player, power, damage, is_critical)
	...
	z_bows.shoot_arrow(arrow_itemstring, {x=playerpos.x,y=playerpos.y+1.5,z=playerpos.z}, dir, yaw, player, power, damage, is_critical, player:get_wielded_item(), not infinity_used)
	return true
end
...
minetest.register_tool(modname .. ":eoaktiakouskou", {
	...
	inventory_image = "z_eoaktiakouskou.png",
...
for level=0, 2 do
	minetest.register_tool(modname .. ":eoaktiakouskou_"..level, {
		description = "Eoaktiakouskou",
		inventory_image = "z_eoaktiakouskou_"..level..".png",
		wield_scale = mcl_vars.tool_wield_scale,
		stack_max = 1,
...
minetest.register_globalstep(function(dtime)
	for _, player in pairs(minetest.get_connected_players()) do
		...
		if type(bow_load[name]) == "number" and ((wielditem:get_name()~=modname .. ":eoaktiakouskou_0" and wielditem:get_name()~=modname .. ":eoaktiakouskou_1" and wielditem:get_name()~=modname .. ":eoaktiakouskou_2" and wielditem:get_name()~=modname .. ":eoaktiakouskou_0_enchanted" and wielditem:get_name()~=modname .. ":eoaktiakouskou_1_enchanted" and wielditem:get_name()~=modname .. ":eoaktiakouskou_2_enchanted") or wieldindex ~= bow_index[name]) then
			reset_bow_state(player, true)
		end
	end
end)
