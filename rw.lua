...
forcegun = (function()
	...
	rw.register_tool(modname .. ":rw_forcegun", {
		...
		on_use = function(itemstack, user, pointed_thing)
			local useriscreative = minetest.is_creative_enabled(user:get_player_name())

			local pos = user:get_pos()
			local dir = user:get_look_dir()
			local yaw = user:get_look_horizontal()
			local inv = user:get_inventory()
			if useriscreative or inv:contains_item("main", modname .. ":rw_power_particle 40") then
				if pos and dir then
					if not useriscreative then
